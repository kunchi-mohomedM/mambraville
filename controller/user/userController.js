...
const loadHomepage = async (req, res) => {
    try {    
        ...
        const trendingProducts=await Products.find({isDeleted:false})
        .sort({quantity:-1})
        .limit(8)
    // console.log(trendingProducts,"gdfhsf")
        const specialOffers=await Products.find({isDeleted:false})
        .sort({quantity:1})
        .limit(3)
       ...
       // console.log(specialOffers)
        ...
        return res.render("home",{newArrivals,trendingProducts,specialOffers});
    } catch (error) {
...
const emailverification=async(req,res)=>{
    try {
        ...
        if(!user){
            return res.render("forgotemailpage",{message:"User with this email is does not exists"})
        }
        const otp = generateOtp();
...
const login = async (req, res) => {
    try {
        ...
        if(!user.password){
            ...
            return res.render("signin",{error:"created a new password using forgot password"})
        }
...
const loadChangePassword = async (req,res) =>{
    try {
        ...
        res.render("changepassword2",{error:null,success:null,activePage:"change-password"});
    } catch (error) {
...
const changePassword = async (req,res) => {
    try {
        ...
        return res.render("changepassword2",{error:null,success:"Password updated successfully!",activePage:"change-password"});
    } catch (error) {
